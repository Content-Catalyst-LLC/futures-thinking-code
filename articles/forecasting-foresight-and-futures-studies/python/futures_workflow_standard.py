#!/usr/bin/env python3
"""
Standard Futures Thinking workflow.

Uses only the Python standard library.

Outputs:
- outputs/strategy_summary_standard.csv
- outputs/strategy_regret_standard.csv
- outputs/practice_profile_scores_standard.csv
- outputs/signal_watch_scores_standard.csv
- outputs/futures_diagnostics_summary.md
"""
        ...
        regret_rows.append(
            {
                ...
                "regret": round(best - performance, 4),
            }
        )

    regrets_by_strategy: dict[str, list[float]] = defaultdict(list)
    for row in regret_rows:
        regrets_by_strategy[str(row["strategy"])].append(float(row["regret"]))
...
def analyze_signals() -> list[dict[str, object]]:
    ...
    for row in rows:
        ...
        watch_score = 0.5 * uncertainty + 0.5 * impact
        output.append(
            {
...
def write_markdown_summary(
    ...
    practice_scores: list[dict[str, object]],
    signal_scores: list[dict[str, object]],
) -> None:
    lines = [
